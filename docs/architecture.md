...
실패하면 입장을 멈추고 새로고침·재시도를 안내합니다. 통화 중 새 배포를 발견하면 안내만
...
구버전 참가자가 보낸 손들기는 배지와 **순서 미지원** 안내로 표시하고 공통 순서를 임의로
매기지 않습니다. 현재 웹은 입장 전에 서버의 `room.hand` 지원을 확인하며, 모두 같은 대기
목록을 사용하려면 참가자 전원이 최신 웹으로 입장해야 합니다.
...
참가자 타일의 **음량** 메뉴에서 0~100% 범위로 내 쪽 재생 음량을 조절합니다.
