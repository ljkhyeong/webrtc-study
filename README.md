...
- 이전 채팅을 읽거나 다른 탭·창에서 작업하는 동안 읽던 위치와 새 메시지 수 유지
- 채팅의 HTTP·HTTPS 링크 열기와 메시지 원문 복사
- 현재 대화 전체 복사(이름·날짜·시간·수신 미확인 상태 포함, 초안 제외)
...
BATON 운영 연동을 준비할 때는 [API 연동과 빌드 준비](docs/integration-readiness.md)의
전용 빌드 명령과 환경변수 예시를 사용합니다. 아래 명령은 독립 실행 개발용입니다.
...
TURN 발급 횟수는 기본 600초 동안 집계합니다. 기본 프로필과 독립 실행 Compose는 IP당 12회,
서버 전체 24회로 제한합니다. `production` 프로필은 BATON의 짧은 참여권에 따른 갱신을 고려해
IP당 36회·서버 전체 72회를 기본으로 사용합니다. 명시한 환경변수가 우선합니다.
TTL이나 브라우저 갱신 시점을 변경하면 발급 구간과 한도도 함께 검토해야
